...
function getBarangaySource(data) {
  return data.barangays || [];
}

function getMunicipalityNameById(municipalities, municipalityId) {
  return municipalities.find((municipality) => municipality.id === municipalityId)?.name || '';
}

function getBarangayNameById(barangays, barangayId) {
  return barangays.find((barangay) => barangay.id === barangayId)?.name || '';
}

function normalizeOfficeLevel(level) {
  const normalized = String(level || '').toLowerCase();

  if (normalized === 'provincial') return 'Provincial Office';
  if (normalized === 'municipal') return 'Municipal Office';
  if (normalized === 'barangay') return 'Barangay Office';

  return level || 'Municipal Office';
}
...
export function buildMunicipalityModuleData(data) {
  ...
  const barangaySource = getBarangaySource(data);
  const users = data.users || [];

  const officeRecords = (data.offices || []).map((office, index) => {
    const officeName = office.name || office.officeName || office.office_name;
    const officeLevel = office.officeLevel || office.office_level || office.type;
    const municipality =
      office.municipality ||
      office.municipalityName ||
      office.municipality_name ||
      getMunicipalityNameById(municipalitySource, office.municipalityId || office.municipality_id) ||
      (officeLevel === 'provincial' ? 'Province of Bulacan' : 'Unassigned');
    const barangay =
      office.barangay ||
      office.barangayName ||
      office.barangay_name ||
      getBarangayNameById(barangaySource, office.barangayId || office.barangay_id);
    const status = office.status || (office.isActive === false || office.is_active === false ? 'Inactive' : 'Active');
    const assignedPersonnel = users.filter(
      (user) => user.role === 'personnel' && user.office === officeName
    );
    const inCharge = office.inCharge || office.lead || assignedPersonnel[0]?.name || 'Not set';
    const programsHandled = (data.programs || []).filter((program) => program.office === officeName);
    const applicationsReceived = (data.applications || []).filter(
      (application) => application.office === officeName
    );

    const fallbackMunicipality = municipalitySource.find((item) => item.name === municipality);

    return {
      id: office.id || `office-generated-${index + 1}`,
      officeId: office.id || `office-generated-${index + 1}`,
      parentOfficeId: office.parentOfficeId || office.parent_office_id || null,
      municipalityId: office.municipalityId || office.municipality_id || null,
      barangayId: office.barangayId || office.barangay_id || null,
      name: officeName,
      type: normalizeOfficeLevel(officeLevel),
      ...
      address:
        office.address ||
        [office.houseNumber || office.house_number, office.streetName || office.street_name, office.subdivisionArea || office.subdivision_area].filter(Boolean).join(' ') ||
        `${municipality || 'Bulacan'} Government Center, ${fallbackMunicipality?.province || 'Bulacan'}`,
      houseNumber: office.houseNumber || office.house_number || '',
      streetName: office.streetName || office.street_name || '',
      subdivisionArea: office.subdivisionArea || office.subdivision_area || '',
      contactNumber: office.contactNumber || office.contact_number || 'Not provided',
      emailAddress:
        office.emailAddress ||
        office.email ||
        `${slugify(officeName)}@bulacan.gov.ph`,
      ...
      description:
        office.description ||
        `${officeName} manages municipality-facing service coordination, program visibility, and application monitoring.`,
      createdAt: office.createdAt || office.created_at || '2026-01-15',
      updatedAt: office.updatedAt || office.updated_at || '2026-03-18',
  ...
  const personnelAssignments = users
    ...
    .map((user, index) => ({
      ...
      assignedOffice: user.office || (user.municipality ? 'Municipality-wide' : 'Unassigned'),
  ...
  const municipalityRecords = municipalitySource.map((municipality, index) => {
    ...
    const barangays = barangaySource
      .filter((barangay) => barangay.municipality === municipality.name)
      .map((barangay, barangayIndex) => ({
        id: barangay.id || `barangay-${municipality.id || index}-${barangayIndex + 1}`,
        barangayId: barangay.id || `barangay-${municipality.id || index}-${barangayIndex + 1}`,
        name: barangay.name || barangay.barangayName || barangay.barangay_name,
        municipality: municipality.name,
        status: barangay.status || (barangay.is_active === false ? 'Inactive' : 'Active'),
        createdAt: barangay.createdAt || barangay.created_at || '2026-01-10',
        updatedAt: barangay.updatedAt || barangay.updated_at || barangay.createdAt || barangay.created_at || '2026-03-18',
      }))
